...
func UpdateComment(e *gin.Engine, db *gorm.DB) {
	e.PATCH("/v1/backend/comment/update", func(context *gin.Context) {
		...
		mp2["id"] = mp1["id"]
		delete(mp1, "id")
		_, err3 := fybDatabase.UpdateSingleCommentByCondition(db, mp2, mp1)
